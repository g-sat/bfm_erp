  ...
  return (
    <div className="mb-4 flex flex-col gap-3 sm:mb-5 sm:flex-row sm:items-end sm:justify-between">
      <div className="min-w-0">
        <h1 className="text-xl font-semibold tracking-tight text-[var(--erp-ink)] sm:text-2xl">
          {title}
        </h1>
        {subtitle ? (
          <p className="mt-1 text-sm text-[var(--erp-muted)] break-words">{subtitle}</p>
        ) : null}
      </div>
      {actions ? (
...
export function Button({
  ...
  return (
    <button
      className={`inline-flex shrink-0 items-center justify-center rounded-md px-3 py-2.5 text-sm font-medium disabled:opacity-50 sm:py-2 ${styles} ${className}`}
      {...props}
    >
      {children}
    </button>
...
export function Table({
  ...
  return (
    <div className="relative">
      <div className="overflow-x-auto overscroll-x-contain rounded-lg [-webkit-overflow-scrolling:touch]">
        <table className="w-full min-w-[36rem] text-left text-sm text-[var(--erp-ink)]">
      ...
      <p className="mt-1 text-[10px] text-[var(--erp-muted)] sm:hidden">Swipe sideways to see more columns</p>
    </div>
  );
}
